...
def build_xosc_preview(data: ScenarioRequest) -> str:
    ...
    Current prototype:
    - Uses pedestrian crossing template
    - Uses esmini demo resources
    - Adds real EnvironmentAction for weather
    - Uses fixed HostSpeed = 10 m/s to match original esmini pedestrian timing
    """

    # esmini expects speed in m/s.
    # For now we keep 10 m/s because this matches the original pedestrian.xosc behavior.
    # Later you can use dynamic conversion again:
    # host_speed_ms = round(data.egoSpeed / 3.6, 2)
    host_speed_ms = 10

    # Weather mapping from UI dropdown to OpenSCENARIO EnvironmentAction
